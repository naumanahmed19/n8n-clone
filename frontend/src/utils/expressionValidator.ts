...
export function validateExpression(value: string): ValidationResult {
  ...
  while ((match = incompleteVarPattern.exec(value)) !== null) {
    ...
    if (lastOpenBrace > lastCloseBrace) {
      warnings.push({
        type: 'warning',
        message: 'Incomplete variable reference. Use $vars.propertyName or $local.propertyName',
        position: match.index,
        length: match[0].length,
      })
    }
 ...
 */
export function getExpressionBlocks(value: string): Array<{ start: number; end: number; content: string }> {
