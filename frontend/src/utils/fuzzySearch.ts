...
export function fuzzyMatch(search: string, target: string): FuzzyMatchResult {
  ...
  if (targetLower === searchLower) {
    return { score: 1, matches: Array.from({ length: search.length }, (_, i) => i) }
  }
  ...
  if (containsIndex !== -1) {
    ...
    return {
      score,
      matches: Array.from({ length: search.length }, (_, i) => containsIndex + i),
    }
  }
  ...
  const compactnessScore = (matches[matches.length - 1] - matches[0] + 1) / target.length
