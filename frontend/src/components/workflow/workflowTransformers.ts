...
export function getNodeExecutionStatus(
    ...
    if (executionState.status === 'success' || executionState.status === 'error' || executionState.status === 'cancelled') {
        if (nodeResult) {
            return nodeResult.status
        }
        
        if (lastExecutionResult) {
            const lastNodeResult = lastExecutionResult.nodeResults.find(nr => nr.nodeId === nodeId)
            if (lastNodeResult) {
 ...
 */
function getNodeOutputs(node: WorkflowNode, nodeTypeDefinition: NodeType | undefined): string[] {
    if (node.type === 'switch' && node.parameters?.outputs) {
        return (node.parameters.outputs as any[]).map((output: any, index: number) => 
 ...
 */
function getNodeCustomStyle(node: WorkflowNode, nodeTypeDefinition: NodeType | undefined) {
...
export function transformWorkflowNodesToReactFlow(
    ...
    return workflowNodes.map(node => {
        ...
        const nodeStatus = getNodeExecutionStatus(node.id, executionState, nodeResult, lastExecutionResult)
 ...
 */
export function transformWorkflowEdgesToReactFlow(connections: WorkflowConnection[]) {
