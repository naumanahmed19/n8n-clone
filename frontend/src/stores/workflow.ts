  ...
  devtools(
    (set, get) => ({
      ...
      executeNode: async (
        ...
        // Create store actions wrapper for handlers
        const storeActions = {
          getState: () => get(),
          setState: (updates: any) => set(updates),
          subscribe: useWorkflowStore.subscribe,
          ...get(),
        };

        // Execute based on mode using dedicated handlers
        if (mode === "workflow") {
          const { WorkflowExecutionHandler } = await import(
            "@/services/execution/WorkflowExecutionHandler"
          );
          await WorkflowExecutionHandler.execute(
            nodeId,
            node,
            workflow,
            storeActions
          ...
          const { SingleNodeExecutionHandler } = await import(
            "@/services/execution/SingleNodeExecutionHandler"
          );
          await SingleNodeExecutionHandler.execute(
            nodeId,
            node,
            workflow,
            nodeInputData,
