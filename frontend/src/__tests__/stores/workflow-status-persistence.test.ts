...
describe('Workflow Store - Status Icon Persistence Fixes', () => {
  ...
  describe('Multiple Execution Handling', () => {
    it('should handle multiple quick executions without status conflicts', async () => {
      ...
      expect(store.getNodeVisualState('node-1').status).toBe(NodeExecutionStatus.COMPLETED);
      expect(store.getNodeVisualState('node-2').status).toBe(NodeExecutionStatus.COMPLETED);
      ...
      expect(store.getNodeVisualState('node-1').status).toBe(NodeExecutionStatus.RUNNING);
      expect(store.getNodeVisualState('node-2').status).toBe(NodeExecutionStatus.IDLE); // Reset for new execution
    });
  ...
  describe('Visual State Preservation', () => {
    it('should preserve node visual states when unsubscribing from execution', async () => {
      ...
      expect(afterNode1State.animationState).toBe(beforeNode1State.animationState);
      expect(afterNode2State.status).toBe(beforeNode2State.status);
      expect(afterNode2State.animationState).toBe(beforeNode2State.animationState);
    });
    ...
    it('should only clear visual states when starting genuinely new execution', () => {
      ...
      expect(store.getNodeVisualState('node-1').status).toBe(NodeExecutionStatus.IDLE);
    });
  });
