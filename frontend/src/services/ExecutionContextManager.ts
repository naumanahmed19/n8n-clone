...
export class ExecutionContextManager {
  ...
  isNodeExecuting(nodeId: string): boolean {
    ...
    for (const executionId of executions) {
      const context = this.executions.get(executionId)
      if (context && context.status === 'running' && context.runningNodes.has(nodeId)) {
        return true
      }
    }
   ...
   */
  getNodeStatusInExecution(nodeId: string, executionId: string): NodeExecutionStatus {
    const context = this.executions.get(executionId)
    ...
    if (context.completedNodes.has(nodeId)) return NodeExecutionStatus.COMPLETED
