...
describe('ExecutionStateManager', () => {
  ...
  it('should start tracking an execution', async () => {
    ...
    expect(mockWebSocket.subscribeToExecution).toHaveBeenCalledWith(executionId)
    expect(mockWebSocket.addEventListener).toHaveBeenCalledWith(executionId, expect.any(Function))
  ...
  it('should stop tracking an execution', async () => {
    ...
    await executionStateManager.startExecution(executionId, nodeIds, dependencies)
    expect(executionStateManager.getExecutionFlowStatus(executionId)).toBeDefined()

    // Stop tracking
    await executionStateManager.stopExecution(executionId)

    expect(mockWebSocket.unsubscribeFromExecution).toHaveBeenCalledWith(executionId)
    expect(executionStateManager.getExecutionFlowStatus(executionId)).toBeUndefined()
  })
