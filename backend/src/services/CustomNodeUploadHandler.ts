...
export class CustomNodeUploadHandler {
  ...
  async processUpload(filePath: string, originalName: string): Promise<UploadResult> {
    try {
  ...
  private async processExtractedContent(extractDir: string): Promise<UploadResult> {
    try {
      ...
      for (const nodeFile of nodeFiles) {
        try {
          const nodeDefinition = await this.processNodeFile(nodeFile, packageInfo!);
          if (nodeDefinition) {
            // Save to database
          ...
          errors.push(`Failed to process ${path.basename(nodeFile)}: ${errorMessage}`);
        }
      }
      ...
      if (processedNodes.length === 0) {
        return {
          ...
          errors: errors.length > 0 ? errors : ['All node files failed validation']
        };
      }
  ...
  private async processNodeFile(filePath: string, packageInfo: PackageInfo): Promise<NodeDefinition | null> {
    try {
      ...
      const nodeInfo = this.extractNodeInfoFromContent(fileContent, path.basename(filePath));

      // Merge information from different sources
      const nodeDefinition: NodeDefinition = {
        type: nodeInfo.type || this.generateTypeFromFilename(filePath),
        displayName: nodeDescription.displayName || nodeInfo.displayName || this.generateDisplayNameFromFilename(filePath),
        name: nodeDescription.name || nodeInfo.name || this.generateNameFromFilename(filePath),
        group: nodeDescription.group || nodeInfo.group || ['Custom'],
        version: nodeDescription.version || packageInfo.version ? parseInt(packageInfo.version.split('.')[0]) : 1,
        description: nodeDescription.description || nodeInfo.description || `Custom node from ${packageInfo.name}`,
  ...
  private extractNodeInfoFromContent(content: string, filename: string): Partial<NodeDefinition> {
