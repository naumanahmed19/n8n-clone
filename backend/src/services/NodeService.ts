...
export class NodeService {
   ...
   * ALL nodes return: {main: [{json: item}, ...], branches?: {...}, metadata: {...}}
   */
  private standardizeNodeOutput(
    ...
    // Process all outputs uniformly
    outputs.forEach((output) => {
      Object.keys(output).forEach((key) => {
        const outputData = output[key] || [];

        if (key === "main") {
          // Standard main output - add to main array
          mainOutput.push(...outputData);
        } else {
          // Branch output (true, false, etc.) - add to branches
          branches[key] = outputData;
          hasMultipleBranches = true;
          // Also add to main for backward compatibility
          mainOutput.push(...outputData);
        }
      });
    });

    const result: StandardizedNodeOutput = {
      main: mainOutput,
      metadata: {
        nodeType,
  ...
  private async registerBuiltInNodes(): Promise<void> {
    ...
    // Import core nodes
    const { IfNode, HttpRequestNode, JsonNode, SetNode } = await import(
      "../nodes/core"
    );

    const builtInNodes = [
      HttpRequestNode.getNodeDefinition(),
      JsonNode.getNodeDefinition(),
      SetNode.getNodeDefinition(),
      IfNode.getNodeDefinition(),
      WebhookTriggerNode,
      ScheduleTriggerNode,
      ManualTriggerNode,
    ];
