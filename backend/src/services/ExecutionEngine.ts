...
export class ExecutionEngine extends EventEmitter {
    ...
    async executeWorkflow(
        ...
        try {
            ...
            logger.info(`Started execution ${execution.id} for workflow ${workflowId}`);
    ...
    async getExecutionStats(): Promise<ExecutionStats> {
        try {
            const [
                ...
                this.prisma.execution.count({ where: { status: ExecutionStatus.RUNNING } }),
                this.prisma.execution.count({ where: { status: ExecutionStatus.SUCCESS } }),
                this.prisma.execution.count({ where: { status: ExecutionStatus.ERROR } }),
                this.prisma.execution.count({ where: { status: ExecutionStatus.CANCELLED } })
            ]);

            const queueSize = await this.executionQueue.getWaiting().then(jobs => jobs.length);
    ...
    private setupQueueProcessors(): void {
        ...
        this.executionQueue.process('execute-workflow', async (job: Job<ExecutionJobData>) => {
     ...
     */
    private async processWorkflowExecution(job: Job<ExecutionJobData>): Promise<void> {
     ...
     */
    private async processNodeExecution(job: Job<NodeExecutionJob>): Promise<NodeOutputData[]> {
        ...
        try {
            ...
            // Emit node-specific event
            this.emitNodeExecutionEvent(executionId, nodeId, 'completed', result.data);
     ...
     */
    private buildExecutionGraph(nodes: Node[], connections: Connection[]): ExecutionGraph {
     ...
     * Wait for node execution to complete
     */
    private async waitForNodeCompletion(executionId: string, nodeId: string): Promise<void> {
    ...
    private async handleNodeExecutionError(
        ...
        if (shouldRetry) {
            ...
            logger.warn(`Retrying node ${nodeId} execution (attempt ${retryCount + 1}) after ${delay}ms`);

            // Schedule retry
            setTimeout(async () => {
                const context = this.activeExecutions.get(executionId);
                if (context && !context.cancelled) {
                    const inputData = this.prepareNodeInputData(nodeId,
            ...
            // Emit node-specific event
            this.emitNodeExecutionEvent(executionId, nodeId, 'failed', { error: error.message });
    ...
    private async emitExecutionProgress(executionId: string): Promise<void> {
        try {
            ...
            if (progress) {
                ...
                logger.debug(`Execution progress: ${progress.completedNodes}/${progress.totalNodes}`, progress);
            }
        } catch (error) {
            logger.error(`Failed to emit execution progress for ${executionId}:`, error);
        }
    }
