...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const whereClause = group 
      ? { group: { has: group as string } }
      : {};
 ...
 */
router.post('/upload', upload.single('nodes'), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
    ...
    const result = await uploadHandler.processUpload(req.file.path, req.file.originalname);
...
router.patch('/:type', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to update node type', { error, type: req.params.type });
    res.status(500).json({
      success: false,
...
router.delete('/:type', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to delete node type', { error, type: req.params.type });
    res.status(500).json({
      success: false,
