...
router.delete('/packages/:packageName', async (req: Request, res: Response) => {
    try {
        ...
        logger.error('Failed to unload package', { error, packageName: req.params.packageName });
        res.status(500).json({
            success: false,
 ...
 */
router.post('/packages/:packageName/reload', async (req: Request, res: Response) => {
    try {
        const { packageName } = req.params;
        ...
        logger.error('Failed to reload package', { error, packageName: req.params.packageName });
        res.status(500).json({
            success: false,
            error: 'Failed to reload package'
        });
    ...
    try {
        ...
        const result = await templateGenerator.generateNodePackage(
            outputPath || process.cwd(),
            {
                ...
                author,
                version,
                group: group ? group.split(',').map((g: string) => g.trim()) : undefined,
                includeCredentials,
                includeTests,
                typescript
            }
...
router.get('/marketplace/search', async (req: Request, res: Response) => {
    try {
        ...
        const marketplace = new NodeMarketplace({
            registryUrl: process.env.NODE_MARKETPLACE_URL || 'https://marketplace.n8n.io'
        });
 ...
 */
router.get('/marketplace/packages/:packageId', async (req: Request, res: Response) => {
    try {
        ...
        logger.error('Failed to get package info', { error, packageId: req.params.packageId });
        res.status(500).json({
            success: false,
    ...
    try {
        ...
        if (result.success) {
            res.json({
                success: true,
                data: result,
                message: dryRun ? 'Package validation successful (dry run)' : 'Package published successfully'
            });
        } else {
            res.status(400).json({
                success: false,
