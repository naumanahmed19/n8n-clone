...
program
  ...
  .option('-t, --type <type>', 'Node type (action, trigger, transform)', 'action')
  ...
  .action(async (options) => {
    try {
      if (!options.name) {
        console.error('Error: Node name is required. Use -n or --name to specify.');
        process.exit(1);
      }
      ...
      const result = await generator.generateNodePackage(options.output, templateOptions);

      if (result.success) {
        console.log(`✅ Node package created successfully at: ${result.packagePath}`);
        ...
        console.log('\n📝 Next steps:');
        console.log(`   cd ${path.relative(process.cwd(), result.packagePath!)}`);
      ...
      if (result.valid) {
        ...
        if (result.packageInfo) {
          ...
          if (result.packageInfo.credentials) {
            console.log(`   Credentials: ${result.packageInfo.credentials.length}`);
          }
        }
...
program
  ...
  .action(async (packagePath, options) => {
    try {
      ...
      // Execute the node
      console.log(`   Executing node: ${nodeDefinition.type}`);
