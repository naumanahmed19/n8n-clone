...
export class UrlSecurityValidator {
    ...
    if (!this.ALLOWED_PROTOCOLS.includes(cleanProtocol)) {
      return {
        ...
        errors: [{
          ...
          details: { protocol: cleanProtocol, allowed: this.ALLOWED_PROTOCOLS }
        }],
        riskLevel: 'medium'
      };
    }
   ...
   */
  public static validateRequestParameters(parameters: Record<string, any>): SecurityValidationResult {
    const errors: SecurityError[] = [];
    let riskLevel: 'low' | 'medium' | 'high' | 'critical' = 'low';

    // Check for potentially dangerous headers
    if (parameters.headers) {
      const dangerousHeaders = ['authorization', 'cookie', 'x-forwarded-for', 'x-real-ip'];
      const headerKeys = Object.keys(parameters.headers).map(k => k.toLowerCase());
      
      for (const dangerousHeader of dangerousHeaders) {
        if (headerKeys.includes(dangerousHeader)) {
          logger.warn('Potentially dangerous header detected', { header: dangerousHeader });
          riskLevel = 'medium';
        }
      }
    }
