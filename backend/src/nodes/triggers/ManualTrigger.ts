  ...
  execute: async function(inputData: NodeInputData): Promise<NodeOutputData[]> {
    ...
    if (allowCustomData) {
      try {
        ...
        this.logger.warn('Failed to parse trigger data, using empty object', { error: error instanceof Error ? error.message : 'Unknown error' });
        customData = {};
      }
      ...
      if (validateData && this.validateTriggerData) {
        ...
        this.logger.warn('Data validation requested but validateTriggerData method not available');
      }
    }
...
declare module '../../types/node.types' {
  interface NodeExecutionContext {
    validateTriggerData?: (data: any, rules: DataValidationRules) => {
      isValid: boolean;
      errors: string[];
      sanitizedData: any;
    };
...
ManualTriggerNode.execute = async function(inputData: NodeInputData): Promise<NodeOutputData[]> {
    ...
    try {
      ...
      if (rules.maxSize) {
        ...
        if (dataSize > rules.maxSize) {
          errors.push(`Data size ${dataSize} bytes exceeds maximum ${rules.maxSize} bytes`);
        }
      }

      // Check data type
      if (rules.allowedTypes && !rules.allowedTypes.includes(typeof data)) {
        errors.push(`Data type '${typeof data}' is not allowed. Allowed types: ${rules.allowedTypes.join(', ')}`);
      }
      ...
      if (rules.requiredFields && typeof data === 'object' && data !== null) {
        const missingFields = rules.requiredFields.filter(field => !(field in data));
      ...
      if (typeof data === 'object' && data !== null) {
        ...
        const foundDangerous = Object.getOwnPropertyNames(data).filter(key => dangerousKeys.includes(key));
        if (foundDangerous.length > 0) {
          errors.push(`Potentially dangerous keys found: ${foundDangerous.join(', ')}`);
        }
          ...
          if (typeof obj === 'object' && obj !== null) {
            Object.getOwnPropertyNames(obj).forEach(key => {
              ...
              if (dangerousKeys.includes(key)) {
                errors.push(`Dangerous key '${key}' found at path '${currentPath}'`);
              }
