      ...
      execute: async function (
        ...
        if (!urlValidation.isValid) {
          const errorMessages = urlValidation.errors
            .map((e) => e.message)
            .join("; ");
          logger.warn("HTTP Request blocked by security validation", {
            url,
        ...
        if (!memoryCheck.isValid) {
          logger.warn("HTTP Request blocked due to memory limits", {
        ...
        if (body && ["POST", "PUT", "PATCH"].includes(method)) {
          ...
          if (!bodySizeCheck.isValid) {
            logger.warn("HTTP Request body size exceeds limits", {
              bodySize,
              ...
              try {
                ...
                if (contentLength) {
                  ...
                  if (!responseSizeCheck.isValid) {
                    logger.warn("HTTP Response size exceeds limits", {
                      responseSize,
