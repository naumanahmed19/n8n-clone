  ...
  try {
    ...
    SwitchNode.properties.forEach((prop, index) => {
      ...
      if (prop.type === "collection") {
        console.log(
          `     Multiple Values: ${prop.typeOptions?.multipleValues}`
        );
        console.log(
          `     Button Text: ${prop.typeOptions?.multipleValueButtonText}`
        );
        console.log(`     Component: ${prop.component}`);

        if (prop.componentProps?.fields) {
          console.log(
            `     Nested Fields (${prop.componentProps.fields.length}):`
          );
          prop.componentProps.fields.forEach((field, i) => {
            console.log(
              `       ${i + 1}. ${field.displayName} (${field.name}) - ${
                field.type
              }`
            );
          });
      ...
      if (prop.options && prop.type === "options") {
        console.log(
          `     Options: ${prop.options.map((o) => o.name).join(", ")}`
        );
      }
    });
    ...
    console.log(
      "   4. Click on Switch node to see the Outputs field with RepeatingField"
    );
  } catch (error) {
