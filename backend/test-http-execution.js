...
async function testHttpExecution() {
  ...
  try {
    ...
    try {
      const singleNodeResponse = await fetch("http://localhost:4000/api/executions/nodes/http-1", {
        method: "POST",
        headers: {
      ...
      console.log("Single node response:", JSON.stringify(singleNodeResult, null, 2));
    } catch (error) {
    ...
    try {
      const workflowResponse = await fetch("http://localhost:4000/api/executions", {
        method: "POST", 
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer your-token-here" // You'll need a valid token
